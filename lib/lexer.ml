...
let skip_white_space lexer =
  let rec skip_white_space' lexer =
    ...
    | Some ch
      ...
      let ch =
        if position >= String.length lexer.input
        then None
        else Some lexer.input.[position]
      in
...
let next_position lexer =
  ...
  ; position
  ; ch =
      (if lexer.position + 1 >= String.length lexer.input
       then None
       else Some lexer.input.[lexer.position + 1])
  }
;;
...
let peek lexer ch ~whenMatched ~default =
  ...
  | _ ->
    ( skip_white_space lexer
    , Some default
      (* Next char could be a white space -> need to advance the lexer on to the next char to evaluate*)
    )
;;
...
let peek_string_literal lexer =
  let rec peek_string_literal' lexer acc =
    match lexer.ch with
    | None ->
      lexer, Some Illegal (* string literal not closed *)
    | Some ch when ch <> '"' ->
      peek_string_literal'
        (next_position lexer)
        (acc ^ String.make 1 ch)
  ...
  in
  let lexer, token = peek_string_literal' lexer "" in
  advance lexer, token
;;
